...
import { monuments } from "../shared/schema";
...
async function seed() {
  console.log('Seeding monuments into Supabase...');

  for (const monument of seedMonuments) {
    const existing = await db.select().from(monuments).where(eq(monuments.name, monument.name));
    
    if (existing.length === 0) {
      console.log(`Adding ${monument.name}...`);
      await db.insert(monuments).values(monument);
    } else {
      console.log(`${monument.name} already exists, skipping.`);
    }
  }
